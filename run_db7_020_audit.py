...
manifest=json.loads((root/'db7-020-manifest.json').read_text())
nb=root/'db7-020-branch-reliability.ipynb'
...
if not ref:
 ...
 slug='db7-020-reliability-'+os.environ['GITHUB_RUN_ID'];ref='beautifulminnd/'+slug
...
if os.environ.get('GITHUB_STEP_SUMMARY'):
 with open(os.environ['GITHUB_STEP_SUMMARY'],'a') as f:f.write('DB7-020 CPU audit: https://www.kaggle.com/code/'+ref+'\n')
...
archives=list((out/'kaggle').rglob('db7_020_branch_reliability.zip'));assert len(archives)==1
...
(out/'verification.json').write_text(json.dumps(done,indent=2));print('DB7-020 VERIFIED COMPLETE',flush=True)

summary=out/'summary';summary.mkdir(exist_ok=True)
with zipfile.ZipFile(archives[0]) as z:
 for name in z.namelist():
  if name.endswith(('.csv','.json','.md','.png')) and not name.endswith('test_trace.csv.gz'):
   target=summary/name

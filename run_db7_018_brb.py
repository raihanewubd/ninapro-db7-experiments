...
NOTEBOOK = "db7-018-brb-full.ipynb"
MANIFEST = "db7-018-manifest.json"
POLL_SECONDS = 90
...
def valid_ref(reference):
    require(re.fullmatch(rf"{ACCOUNT}/db7-018-brb-full-[0-9]+-[0-9]+", reference) is not None,
            "Expected the recorded beautifulminnd/db7-018-brb-full-RUNID-ATTEMPT reference")
    return reference
...
def prepare(root, output):
    ...
    slug = f"db7-018-brb-full-{run_id}-{attempt}"
...
def validate_archive(path, expected_manifest=None):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        evaluation = read_json("evaluation/completion.json")
        require(evaluation.get("success") is True and evaluation.get("subjects") == SUBJECTS,
                "Full evaluation coverage missing")
        for subject in SUBJECTS:
            suffix = f"/S{subject:02d}/seed_42/meta/meta_rule_initial.csv"
            require(any(n.startswith('full/') and n.endswith(suffix) for n in names),
                    f"Missing initial rules for subject {subject}")
        ...
        require(len(full_names) == EXPECTED_FITS, f"Expected 374 full fit manifests; found {len(full_names)}")
        identities = set()
    ...
    return dict(success=True, experiment_id=EXPERIMENT, archive=Path(path).name,
                ...
                meta_completed=len(SUBJECTS), subjects=SUBJECTS, seeds=SEEDS,
...
def run(root, monitor_ref="", prepare_only=False):
    ...
    if os.environ.get("DB7_SUBMIT_ONLY") == "1":
        print("Submitted; subsequent workflow jobs monitor the same reference.")
        return
    try:
        monitor(reference, output, state)
    except TimeoutError:
        if os.environ.get("DB7_PENDING_OK") == "1":
            state.update(state="still_running", updated_utc=utc_now())
            write_json(state_path, state)
            return
        raise
    except Exception as error:
        ...
        if state.get("state") != "still_running":
    ...
    expected_manifest = json.loads((root / MANIFEST).read_text(encoding="utf-8"))
    try:
    ...
    write_json(state_path, state)
    print("Verified DB7-018: 374 full 13-epoch fits and two completed meta-learning reports.", flush=True)
    if summary:
        with open(summary, "a", encoding="utf-8") as stream:
            stream.write("\nVerified: ZIP CRC, all 374 full histories, both GPUs, smoke preflight and two subject meta-completions.\n")

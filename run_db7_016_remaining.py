...
resume_ref=os.environ.get('KAGGLE_RESUME_REF','beautifulminnd/db7-016-remaining-64-fits').strip()
if resume_ref:
 assert resume_ref.startswith(username+'/')
 ref=resume_ref
shutil.copyfile(source,folder/'experiment.ipynb')
metadata=dict(id=ref,title=ref.split('/')[-1],code_file='experiment.ipynb',language='python',kernel_type='notebook',is_private=True,enable_gpu=True,enable_internet=False,machine_shape='NvidiaTeslaT4',dataset_sources=['rayaanraza1/ninapro-db7'],competition_sources=[],kernel_sources=[],model_sources=[])
...
record['status']='monitoring existing run' if resume_ref else 'submitted';(out/'submission.json').write_text(json.dumps(record,indent=2))

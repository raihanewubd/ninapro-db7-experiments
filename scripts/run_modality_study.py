...
ARMS = {'emg': ('emg',), 'acc': ('acc',), 'gyroscope': ('gyro',), 'magnetometer': ('mag',),
        'emg_acc': ('emg','acc'), 'emg_acc_gyroscope': ('emg','acc','gyro'),
        'emg_acc_magnetometer': ('emg','acc','mag')}
...
def prepare(job):
    ...
    path = ROOT/'notebooks'/('db7-'+('emg' if job=='smoke' else job.replace('_','-'))+'.ipynb')

"""Submit, collect and verify the paired DB7 energy ablation study.
...
def job_settings(job):
    ...
    if not match:
        raise ValueError("ENERGY_JOB must be smoke, seed42, seed43 or seed44")
...
def prepare(job):
    ...
    slug = f"db7-energy-{run_id}-{attempt}-{job}"
    reference = f"{user}/{slug}"
    source = ROOT / "notebooks" / "db7-energy.ipynb"
    ...
    override = (
        ...
        "RESULTS_DIRECTORY = run_energy_study()\n"
    )
    ...
    folder = ROOT / "energy_submitted" / job
...
def validate_archive(path, job, reference=None, expected_provenance=None):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        assert manifest["trim_ms"] == 100 and manifest["split_seed"] == 42
        assert manifest["labels"] == list(range(1, 18))
        assert manifest["modalities"] == ["emg", "acc", "gyro", "mag"]
        assert not any(manifest[key] for key in ("acc_centering", "gating", "augmentation"))
        ...
        for subject in subjects:
            for arm in ARMS:
                ...
                assert fit["parameters"] == {"C1": 551542, "C2A": 555510, "C2D": 555510}[arm]
                ...
                assert identity == hashes.setdefault(subject, identity), "Energy-arm window identities differ"
                shared = fit["initial_shared_parameter_sha256"]
                assert re.fullmatch(r"[a-f0-9]{64}", shared)
                assert shared == shared_initializations.setdefault(subject, shared), "Shared initial parameters differ"
                for filename in ["best_model.pt", "normalizer.npz", "history.csv", "metrics.csv", "spectral_scaler.npz"]:
                    assert f"{prefix}/{filename}" in names, f"Missing {prefix}/{filename}"
                if arm != "C1":
                    for filename in ["energy_scaler.npz", "energy_scaler.json"]:
                        assert f"{prefix}/{filename}" in names, f"Missing {prefix}/{filename}"
...
def validate(folder, job, reference, provenance):
    archives = list(folder.rglob("db7_energy_*.zip"))
...
def run_job(job):
    ...
    output = ROOT / "energy_results" / job
    ...
    try:
        ...
        write_json(state_file, launch_state)
        deadline = time.monotonic() + (60 if smoke else 320) * 60
        failures = 0
        while True:
        ...
        for retry in range(3):
            ...
            if code == 0 and list(output.glob("db7_energy_*.zip")):
                break
            if retry < 2:
                time.sleep(30)
...
def github_request(url, token):
    return urllib.request.Request(url, headers={
        ...
        "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "db7-energy-analysis",
    })
...
def collect_analysis():
    ...
    for page in range(1, 101):
        ...
        if len(batch) < 100:
            break
    downloads = ROOT / "energy_analysis_downloads"
    ...
    for seed in SEEDS:
        job = f"seed{seed}"
        name = f"db7-energy-{run_id}-{attempt}-{job}"
        ...
        with zipfile.ZipFile(outer_path) as outer:
            members = [m for m, _ in safe_members(outer) if PurePosixPath(m.filename).name.startswith("db7_energy_") and m.filename.endswith(".zip")]
        ...
        checked = validate_archive(inner_path, job, expected_provenance={
            ...
            "source_sha256": file_hash(ROOT / "notebooks" / "db7-energy.ipynb"),
        })
        ...
        print(f"Collected and verified {job}: 66 full fits", flush=True)
    ...
    write_json(collected / "collection_verification.json", dict(
        success=True, cnn_fits=198, seeds=SEEDS, arms=ARMS,
        github_run_id=run_id, github_run_attempt=attempt, sources=verifications,
    ))
    subprocess.run([
        sys.executable, str(ROOT / "scripts" / "analyze_energy_results.py"),
        "--roots", str(collected), "--output", str(ROOT / "energy_analysis"), "--require-complete",
    ], check=True)
...
def main():
    ...
    if args.collect_analysis:
        ...
        run_job(os.environ.get("ENERGY_JOB", "smoke"))

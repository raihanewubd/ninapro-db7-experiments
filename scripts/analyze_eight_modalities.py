...
EXTRA_RUN=os.environ['EXTRA_RUN_ID']
items=json.loads(subprocess.check_output(['gh','api',f'repos/{REPO}/actions/runs/{RUN}/artifacts?per_page=100']))['artifacts']
items += json.loads(subprocess.check_output(['gh','api',f'repos/{REPO}/actions/runs/{EXTRA_RUN}/artifacts?per_page=100']))['artifacts']
ARMS.append('all_sensors')
metrics=[];preds={};fits=[];keys={};sources=[]
for arm in ARMS:
 source_run=EXTRA_RUN if arm=='all_sensors' else RUN
 source_attempt=os.environ.get('EXTRA_RUN_ATTEMPT','1') if arm=='all_sensors' else '1'
 item=next(x for x in items if x['name']==f'db7-modalities-{source_run}-{source_attempt}-{arm}')
 dest=Path(arm+'.zip')
 ...
 with zipfile.ZipFile(dest) as outer:
  ...
  with zipfile.ZipFile(io.BytesIO(outer.read(inner_names[0]))) as z:
   ...
   assert manifest['automation']['github_run_id']==source_run
...
for a,b in [('emg_acc','emg'),('emg_acc','acc'),('emg_acc_gyroscope','emg_acc'),('emg_acc_magnetometer','emg_acc'),('all_sensors','emg_acc_magnetometer')]:
...
fig,ax=plt.subplots(figsize=(11,8));im=ax.imshow(t.values,vmin=40,vmax=100,cmap='viridis',aspect='auto');ax.set_xticks(range(len(ARMS)),ARMS,rotation=35,ha='right');ax.set_yticks(range(22),['S'+str(x) for x in t.index]);fig.colorbar(im,ax=ax,label='Test accuracy (%)');fig.tight_layout();fig.savefig(OUT/'subjects.png',dpi=160);plt.close(fig)
def table(frame):return frame.round(3).to_markdown()
report=f'''# DB7 modality results — run {RUN}
All eight full runs verified: 176 fits. New all-sensor source run: {EXTRA_RUN}. Identical train/validation/test window identities across arms. Exercise B E1 labels 1–17, within-subject 4/1/1 repetitions, 400 ms window, 100 ms stride. One seed per subject. No gate, no augmentation, no per-window ACC centering.
...
## Shared failures
Test windows per arm: {len(p)}. Windows wrong in every arm: {int(p.all_wrong.sum())}. Hypothetical perfect selection among eight predictions: {common.oracle_percent.mean():.3f}% mean-subject accuracy. This is a hindsight bound, not a deployable classifier or signal-information ceiling.
{table(common)}
...
'''
cohort=d.copy()
cohort['cohort']=np.where(cohort.subject<=20,'Non-amputee S1-S20','Amputee S21-S22')
grouped=cohort.groupby(['modality','cohort','split'])[['accuracy','f1_macro']].mean()*100
grouped.to_csv(OUT/'cohort_metrics.csv')
group_table=cohort[cohort.split=='test'].groupby(['modality','cohort']).accuracy.mean().unstack()*100
group_table['All 22 subjects']=t.mean();group_table['S21']=t.loc[21];group_table['S22']=t.loc[22]
group_table=group_table.reindex(ARMS);group_table.to_csv(OUT/'cohort_test_accuracy.csv')
fig,ax=plt.subplots(figsize=(12,6));group_table[['Non-amputee S1-S20','Amputee S21-S22']].plot.barh(ax=ax);ax.set(xlabel='Mean subject test accuracy (%)',xlim=(0,100));fig.tight_layout();fig.savefig(OUT/'cohort_accuracy.png',dpi=160);plt.close(fig)
report='## Cohort comparison\n\n'+table(group_table)+'\n\nCohorts follow the user-specified S1-S20 / S21-S22 grouping. Each subject retains its original within-subject trained model. These are averages of existing predictions, not separate pooled cohort training. Only two amputee subjects: show both individuals and do not generalize a population-level group difference.\n\n'+report

...
def prepare(scope):
    ...
    title=f'DB7 B EDA {rid} {attempt} {scope}';ref=username+'/'+title.lower().replace(' ','-')
    folder=ROOT/'eda_submitted'/scope;folder.mkdir(parents=True,exist_ok=True)
    notebook=json.loads((ROOT/'notebooks/db7-exercise-b-eda.ipynb').read_text(encoding='utf-8'))
...
def validate(results,scope,ref):
    archives=list(results.rglob('db7_exercise_b_eda_*.zip'))
    ...
    with zipfile.ZipFile(archives[0]) as z:
        ...
        expected={(s,'shrinkage_lda',m) for s in subjects for m in ['emg','acc','both']} | {(s,'original_multikernel_cnn','both') for s in subjects}
        actual=[(int(r['subject']),r['model'],r['modality']) for r in rows]
        assert set(actual)==expected and len(actual)==len(expected)
        for s in subjects:
            for suffix in ['repetition_inventory.csv','gesture_feature_shift.csv','cnn_baseline/failure_cases.csv','cnn_baseline/class_learning_history.csv','cnn_baseline/selected_input_gradients.npz']:
                assert f'S{s:02}/{suffix}' in z.namelist(),f'Missing S{s:02}/{suffix}'
        for name in ['model_summary.csv','completion.json','run_manifest.json','READ_ME_RESULTS.md']:(results/name).write_bytes(z.read(name))
...
def main():
    ...
    requested=os.environ.get('EDA_SCOPE','all')
    if requested not in ('smoke','all'):raise ValueError('EDA_SCOPE must be smoke or all')
    ...
    for scope in (['smoke','all'] if requested=='all' else ['smoke']):
        ...
        results=ROOT/'eda_results'/scope;results.mkdir(parents=True,exist_ok=True)
        ...
        try:
            ...
            wait(ref,results,30 if scope=='smoke' else 180)
        finally:
            for retry in range(3):
                try:
                    ...
                    if code==0 and list(results.rglob('db7_exercise_b_eda_*.zip')):break
